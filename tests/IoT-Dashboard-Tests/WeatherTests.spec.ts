...
test.describe("IoT Dashboard - Weather Component Tests", () => {
  ...
  test.describe("Weather Component Layout", () => {
    test("should display weather card with all weather information", async ({
      IoTDashboardPage,
    }) => {
    ...
    test("should display weather icon", async ({ IoTDashboardPage }) => {
      ...
      await test.step("Verify icon has proper styling", async () => {
        const iconClass =
          await IoTDashboardPage.weatherIcon.getAttribute("class");
        expect(iconClass).toContain("today-icon");
      });
    });

    test("should display current weather details", async ({
      IoTDashboardPage,
    }) => {
      ...
      await test.step("Verify max temperature parameter", async () => {
        const maxTempElement = IoTDashboardPage.page
          .locator(".parameter")
          .filter({ hasText: "max" });
        await expect(maxTempElement).toBeVisible();

        const maxTempValue = await maxTempElement
      ...
      await test.step("Verify min temperature parameter", async () => {
        const minTempElement = IoTDashboardPage.page
          .locator(".parameter")
          .filter({ hasText: "min" });
        await expect(minTempElement).toBeVisible();

        const minTempValue = await minTempElement
          .locator(".parameter-value")
          .textContent();
      ...
      await test.step("Verify wind parameter", async () => {
        const windElement = IoTDashboardPage.page
          .locator(".parameter")
          .filter({ hasText: "wind" });
        await expect(windElement).toBeVisible();

        const windValue = await windElement
          .locator(".parameter-value")
          .textContent();
      ...
      await test.step("Verify humidity parameter", async () => {
        const humElement = IoTDashboardPage.page
          .locator(".parameter")
          .filter({ hasText: "hum" });
        await expect(humElement).toBeVisible();

        const humValue = await humElement
          .locator(".parameter-value")
          .textContent();
  ...
  test.describe("Weekly Forecast", () => {
    test("should display weekly forecast with multiple days", async ({
      IoTDashboardPage,
    }) => {
    ...
    test("should display proper day names in forecast", async ({
      IoTDashboardPage,
    }) => {
      await test.step("Verify day names are displayed correctly", async () => {
        const dayNames =
          IoTDashboardPage.weeklyForecast.locator(".day .caption");
    ...
    test("should display weather icons for each forecast day", async ({
      IoTDashboardPage,
    }) => {
      await test.step("Verify weather icons are present", async () => {
        const weatherIcons =
          IoTDashboardPage.weeklyForecast.locator(".weather-icon");
      ...
      await test.step("Verify icons have proper classes", async () => {
        const firstIcon = IoTDashboardPage.weeklyForecast
    ...
    test("should display temperatures for forecast days", async ({
      IoTDashboardPage,
    }) => {
      await test.step("Verify temperatures are displayed", async () => {
        const temperatures =
          IoTDashboardPage.weeklyForecast.locator(".temperature");
  ...
  test.describe("Weather Component Data Validation", () => {
    test("should display realistic weather data", async ({
      IoTDashboardPage,
    }) => {
      await test.step("Verify temperature values are realistic", async () => {
        ...
        const tempValue = parseInt(currentTemp.replace("°", ""));
      ...
      await test.step("Verify wind speed is realistic", async () => {
        ...
        const windValue = await windElement
          .locator(".parameter-value")
          .textContent();
        const windSpeed = parseInt(windValue?.split(" ")[0] || "0");
      ...
      await test.step("Verify humidity is within valid range", async () => {
        ...
        const humValue = await humElement
          .locator(".parameter-value")
          .textContent();
        const humidity = parseInt(humValue?.replace("%", "") || "0");
