...
test.describe('UI Component Tests', () => {
  ...
  test.describe('Form Layouts Page', () => {
    ...
    test('Input Fields', async ({ page }) => {
      ...
      const usingTheGridEmailInput = getFormElement(page, 'Using the Grid', 'textbox', 'email');
    ...
    test('Radio Buttons', async ({ page }) => {
      ...
      const radioOption = usingTheGridForm.getByRole('radio', { name: "Option 1" });
  ...
  test.describe('Tooltips', () => {
    test('Tooltip shows on hover', async ({ page }) => {
      ...
      const tooltipButton = page.locator('nb-card')
  ...
  test.describe('Smart Table', () => {
    ...
    test('Edit table row data', async ({ page }) => {
      ...
      const firstPageRow = page.locator('table').locator('tr', { hasText: 'twitter@outlook.com' }).first();
      ...
      const secondPageRow = page.getByRole('row', { name: '11' })
        .filter({ has: page.locator('td').nth(1).getByText('11') });
      await editTableCell(page, secondPageRow, 'E-mail', 'test@test.com');
      
      // Verify edit was successful
      await expect(secondPageRow.locator('td').nth(5)).toHaveText('test@test.com');
    });
 ...
 */
async function navigateToPage(page: Page, menuItem: string, subMenuItem: string): Promise<void> {
 ...
 * Get an element from a specific form
 */
function getFormElement(page: Page, formName: string, role: string, name: string): any {
    return page.locator('nb-card').filter({ hasText: formName }).getByRole(role as any, { name });
  }
...
async function selectTheme(page: Page, themeName: string): Promise<void> {
  const dropDownMenu = page.locator('ngx-header nb-select');
  await dropDownMenu.click();
  await page.locator('nb-option-list nb-option').filter({ hasText: themeName }).click();
}

/**
 * Verify the header background color
 */
async function verifyHeaderColor(page: Page, expectedColor: string): Promise<void> {
 ...
 */
async function editTableCell(page: Page, row: any, placeholder: string, value: string): Promise<void> {
